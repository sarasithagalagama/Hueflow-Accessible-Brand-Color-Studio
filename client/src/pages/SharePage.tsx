...
export function SharePage() {
  ...
  const gradient = gradients[selectedIndex] ?? gradients[0];
  ...
  return <div className="share-page">
    ...
    {gradients.length > 1 && <div className="shared-gradient-tabs" aria-label="Shared gradients">{gradients.map((item, index) => <button className={index === selectedIndex ? "active" : ""} key={item.id} onClick={() => setSelectedIndex(index)}>{item.name}</button>)}</div>}
